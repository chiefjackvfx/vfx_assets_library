...
On Windows, `run_vfx_asset_library.bat` is the only file needed. Double-click it or run it from Command Prompt: when necessary it installs an official per-user Python 3.13 runtime, downloads `main` from the approved GitHub repository, pins the archive to its commit ID, validates its contents, creates the virtual environment, installs dependencies, and opens ShotBox Assets. Application versions live under `%LOCALAPPDATA%\ShotBoxAssets\application`, Python under `%LOCALAPPDATA%\ShotBoxAssets\runtime`, and the most recent validated version remains available when GitHub is offline. A clean Git `main` checkout instead uses a fast-forward update from `origin/main`; tracked local changes and diverged branches are never overwritten. Pass `--no-update` for one launch or set `SHOTBOX_AUTO_UPDATE=0` to run an already complete local copy without checking. Private-repository downloads can use a `SHOTBOX_GITHUB_TOKEN` environment variable with read-only Contents permission; neither that token nor administrator access is required for a public repository.
