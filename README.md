...
The `render_ball` camera produces `_Texture_Thumbnail.jpg` for catalog cards, while `render_plane` produces `_Texture_Preview.jpg` for the inspector hero. During import, Blender is requested only when the source contains no usable provider preview; an existing thumbnail or hero is retained unchanged. Desktop imports publish the asset first and place missing texture and HDRI renders into one background queue. **Parallel Blender preview renders** in Settings allows one to 36 simultaneous Blender instances and defaults to two; each worker keeps its Blender process alive and reuses it for later texture or HDRI jobs. More instances require additional GPU memory and system RAM and can reduce throughput when the GPU is saturated. Manual inspector requests move ahead of pending automatic jobs without interrupting active renders. Multiple texture or HDRI cards can also be selected and added together with **Queue Previews**. Queued and rendering cards display an animated loading circle, the toolbar reports total and active work and can clear pending work, and selecting a queued or rendering asset allows that individual job to be canceled. Queue errors remain non-blocking and processing continues with the next asset. Completed files and manifests are still published one at a time under the library lock, while the expensive Blender renders run concurrently. Missing-preview rendering has its own default-on Settings preference. A separate default-off debugging preference saves a self-contained `_Texture_Preview.blend` beside generated images in the managed asset's `previews/` folder; the material maps are packed into it so the exact render state can be opened after temporary import files are removed. Blender remains optional: a failed or unavailable render never fails the texture import. Texture details expose **Render preview** for a retained source image, or **Regenerate preview** after a shader render, with progress, cancellation, and diagnostics. Manual rendering verifies the unchanged manifest and every selected map hash before atomically publishing both images and, when enabled, the debug scene. Atlases retain their source previews.

## Houdini VDB still previews

VDB details provide a manual Low/Mid/High variant selector, a Karma Pyro density slider from 10–500 (default 100), and a **Generate preview** menu. **Still** renders timeline frame 1 to the existing JPEG preview. **Generate turn table** renders the template timeline from frame 1 through 36, uses the first frame as the JPEG hero/thumbnail, and encodes all 36 frames into a managed H.264 MP4 at the HIP frame rate. ShotBox launches Houdini 22 or newer headlessly, opens a staging copy of `templates/VDB_preview_v001.hip`, assigns the managed file to `/obj/VDB/file1`, sets `kma_pyroshader1`'s Density Scale, and renders `/stage/usdrender_rop1`. The authoritative template remains untouched and controls camera, animation, lighting, materials, Karma settings, sampling, and output resolution. Any readable resolution is accepted; odd dimensions are padded by at most one pixel for H.264 compatibility. Sequence variants use their padded `$F` expression and must contain source frame 1.

Choose Houdini, hbatch, or hython in **Settings**, or leave the path empty to search installed Houdini 22+ versions. **Check Houdini** verifies the version and companion `iconvert` tool; turntables also require the configured or auto-detected FFmpeg. **Parallel VDB turntable renders** allows one to 36 simultaneous Houdini instances (one per output frame) and defaults to two. The 36 frames are divided into contiguous ranges, each instance loads a separate staged HIP, and any worker failure or cancellation stops the complete group before encoding or publication. More instances require sufficient Houdini licenses, RAM, and GPU capacity. Temporary HIP copies, EXRs, and encoding files are written beneath the application folder's local `temp_cache/vdb-preview-<id>/` directory, keeping frame traffic off the managed library or network share. The job directory is discarded after success, failure, or cancellation; only the final JPEG and MP4 are atomically copied into the managed asset. VDB jobs share the preview queue but remain exclusive: earlier Blender jobs finish and their sessions close before Houdini starts, and later Blender work resumes only after the VDB job ends. VDB previews are manual-only, so imports and bulk **Queue Previews** do not enqueue them without an explicit VDB action. Cancellation terminates every active headless process tree. Failed, canceled, timed-out, or stale renders retain the previous preview, and neither the template, VDB, nor any HIP file is saved or modified.

Bulk-selected VDBs also provide **Deadline Husk turntables…**. ShotBox first opens a batch panel for the requested variant and Pyro density, then uses the template's existing `/stage/usd_rop1` to export one time-sampled USD per asset for frames 1–36. The ROP flattens the composed stage and writes SOP volume sidecars beside the USD, preventing Linux-only template sublayers from breaking Windows workers. Each USD and its authored EXRs use a unique `previews/deadline/<submission-id>/` folder on the shared asset library. One external Husk Standalone submitter panel opens with every successful USD, leaving pool, renderer, chunk size, and overrides under Deadline control. ShotBox records the pending generation in the asset manifest, resumes watching after restart, and waits for all 36 EXRs to remain stable across consecutive checks. It then publishes the normal JPEG hero and H.264 turntable atomically and removes the farm USD/EXRs. Failed or stale finalization retains the previous preview and farm files for retry. **Resubmit Deadline USD**, **Retry Deadline finalization**, and **Stop watching Deadline preview** are available from the VDB preview menu; stopping the watch neither cancels Deadline nor removes files workers may still be writing. Configure and validate `deadlinecommand` and `HuskStandaloneSubmission.py` in Settings.
...
ShotBox Assets can send imported HDRIs, PBR texture materials, managed USD or FBX models, and VDB volumes into running Blender 5.1/5.2 and Houdini 21/22 sessions. VDB sending is Houdini-only in this release:
...
For a USD-ready model, choose the preferred or another managed USD variant and **Import model**. Blender imports editable geometry and authored materials directly into the current collection, offsets the imported top-level objects to the 3D cursor, preserves source hierarchy, and creates no ShotBox collection or wrapper empty. Each send remains a separate placement and excludes USD cameras and lights. Houdini can create a live Reference LOP in Solaris or a packed USD File Import SOP in the selected geometry network, falling back to `/stage` or a new `/obj` Geometry container as needed. SOP imports also build managed MaterialX shaders under `/mat` and append a Material SOP assignment; Normal takes priority over Bump and Displacement remains separate. DCC scenes are never saved automatically and managed library files are never copied or modified by the bridge.

Managed FBX variants appear alongside USD variants and can be sent directly without first converting to USD. Blender creates an editable snapshot directly in the current collection, with no added collection or wrapper empty. Houdini creates an isolated `/obj` subnet whose unlocked File SOPs remain live references to the managed FBX; FBX sends are SOP-only. Static mesh geometry and hierarchy are supported, while cameras and lights are excluded and Blender rejects rigs, skinning, animation, and blend shapes. When managed texture sets exist, normalized FBX material names are matched to those sets and rebuilt with the same complete Principled or MaterialX graphs as texture sends; a single source material and texture set may fall back to each other. Ambiguous multi-material sends fail without leaving partial scene data. Assets without managed texture sets import as geometry only. Reinstall the Blender plugin and restart Blender after updating ShotBox Assets to receive the simplified import behavior.
...
- Blender model imports are editable snapshots; Houdini LOP, packed-USD SOP, and FBX SOP imports retain their external managed-file reference.
...
- recognizes Megascans (including legacy `maps` and `components` records without `semanticTags`) and Poly Haven metadata;
...
Model mode recognizes USD/USD variants, FBX, OBJ, Alembic, glTF/GLB, Blender, and Maya scenes. One selected source folder becomes one model asset with a uniform managed layout: model files are flattened into `models/`, texture maps into `maps/<resolution>/`, previews into `previews/`, provider JSON into `metadata/`, and ordinary companions into `extras/`. Source subfolders are not recreated. Preview discovery requires explicit evidence from an exact provider-metadata path, a common preview/thumbnail/gallery folder, or preview-like filename wording such as Preview, Thumbnail, Hero, Render, or Beauty; JPG, JPEG, PNG, and WebP are supported. If only one eligible image exists it is used for both the hero and thumbnail. Texture channels such as Thickness, texture-folder images, logos, licenses, wireframes, and unlabelled support artwork cannot become model previews. Duplicated metadata basenames fall back to path and filename evidence instead of silently choosing the wrong file. Assets without a real preview keep empty preview assignments and use ShotBox's built-in catalog placeholder; no placeholder bitmap is copied into the managed asset. USD is always preferred when locally available and is marked **USD Ready**. Assets without USD still import using the best available Blender, Maya, FBX, OBJ, Alembic, or glTF representation and are marked **No USD**.

VDB mode scans `.vdb` files without loading OpenVDB grids. Quality tokens such as Low, Mid, and High become selectable variants on one asset. A numeric run after a quality token is treated as a sequence frame only when multiple matching frames exist; static numbered collections such as `cloud_formation_001_Low_Res.vdb` remain separate assets. Sequence padding, source frame range, and gaps are preserved in `asset.json`. New imports store all LOD files directly in `vdbs/<category>/<asset>/volumes/`, with variant grouping retained in the manifest. Filename collisions receive a variant prefix while keeping sequence frame patterns intact. Existing assets with variant subfolders remain readable. Supplied JPG, PNG, WebP, MOV, and MP4 previews are paired by normalized names; FFmpeg extracts a midpoint thumbnail when available, otherwise the retained video uses a volume placeholder. Portable VDB categories are stored in `.ual/vdb_categories.json`.
...
The Settings tab includes confirmed **Update / Fix Library** and **Rename existing assets** actions. Its timestamped Maintenance activity log reports inspections, each asset operation, preview and metadata repairs, failures, cancellation, cleanup, and the final totals instead of showing only a changing summary label. Update/Fix validates manifests, converts older HDRI/model layouts, reorganizes legacy Poly Haven USD downloads, and safely flattens legacy per-asset Stock folders. It also detects readable JPG, JPEG, PNG, or WebP files manually placed directly in an asset's `previews/` directory when their names contain an explicit Preview, Hero, Render, Thumbnail, or Thumb role. The existing file is registered in `asset.json` without being copied; one hero image may serve both roles, while a dedicated thumbnail takes precedence when present. Obsolete generated `*Placeholder*` files are unregistered and removed because the catalog uses its shared built-in placeholder instead. Existing Stock metadata and extras are preserved with asset-prefixed loose filenames during migration. All updates use the same lock, staging, validation, and rollback safeguards.
