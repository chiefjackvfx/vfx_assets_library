#!/usr/bin/env python3
"""Safely update a ShotBox Assets Git checkout for the Windows launcher."""
...
def attempt_update(
    ...
    *,
    ...
    if target_result.returncode:
        return UpdateResult(
            "unavailable",
            f"Update check did not find {remote}/{branch}; starting the installed version.",
            before=before,
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    project = values.project.resolve()
    if not is_project_checkout(project):
        print("Automatic update skipped: the ShotBox Assets checkout is incomplete.", file=sys.stderr)
        return 1
    ...
    result = attempt_update(project)
    print(result.message)
    if result.state != "updated":
        return 0
    try:
        relaunch_windows(values.launcher.resolve(), values.application_arguments, project)
